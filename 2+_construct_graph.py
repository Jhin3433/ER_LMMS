...
#2_consturct_graph：一个一个添加node，2+批量增加node

class event_sense_graph:
    def __init__(self) -> None:
        if os.path.exists("results_save/2+_event_sense_mapping_graph.gpickle"):
            self.G = nx.read_gpickle("2+_results_save/event_sense_mapping_graph.gpickle")
        else:
    ...
    def multi_graph_from_json(self, dir_name = "./results_save/event_sense_mapping_json"):
        ...
        nx.write_gpickle(self.G, "results_save/2+_event_sense_mapping_graph.gpickle")

    

if __name__ == "__main__":
    logging.basicConfig(filename='2+_construct_graph.log', format='%(asctime)s | %(levelname)s | %(message)s', level=logging.DEBUG, filemode='w') #有filename是文件日志输出,filemode是’w’的话，文件会被覆盖之前生成的文件会被覆盖
